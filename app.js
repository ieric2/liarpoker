...
function setupRound(gameId, realId) {
    ...
    for (var i in gameList[gameId].players) {
        ...
        io.to(playerId).emit("newRound", {
            ...
            lives: playerList[playerId].lives,
        });
...
function checkHandIncreases(data) {
    ...
    if (handType < prevHandType) {
        ...
        switch (prevHandType) {
            ...
            case 2: //twoPair
                return (
                    Math.max(prevHandSubtype, prevHandSubtype2) < Math.max(data[1], data[2]) ||
                    (Math.max(prevHandSubtype, prevHandSubtype2) == Math.max(data[1], data[2]) &&
                        Math.min(prevHandSubtype, prevHandSubtype2) < Math.min(data[1], data[2]))
                );
            case 1: //pair
                return prevHandSubtype < data[1];
            case 0: //highCard
    ...
    socket.on("playTurn", function (data) {
        if (socket.realId == playerArray[gameList[socket.gameId].playerTurn]) {
            ...
            if (!error && (gameList[socket.gameId].turnArray.length == 0 || checkHandIncreases(data))) {
                gameList[socket.gameId].turnArray.push([
                    playerList[socket.realId].name,
                    data[0],
                    data[1],
                    data[2],
                ...
                io.to(data.gameId).emit("updateGame", {
                    newPlayerTurn: playerList[gameList[socket.gameId].players[newTurn]].name,
                    pastMove: gameList[socket.gameId].turnArray[gameList[socket.gameId].turnArray.length - 1][4],
                });
                updatePlayerArray(data.gameId, socket.realId);
            } else {
        ...
        if (gameList[data.gameId].playerTurn == null) {
            ...
            io.to(data.gameId).emit(
                ...
                "<b> " +
                    ...
                    doubtValidity +
                    " doubted " +
                    gameList[data.gameId].turnArray[gameList[data.gameId].turnArray.length - 1][0] +
                    "'s hand </b>"
            );
            for (i in playerList) {
